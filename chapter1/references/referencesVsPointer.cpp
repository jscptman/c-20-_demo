// 下面比较引用和指针, 实现同一功能时的区别:

// 指针
#include <cstddef>
#include <iostream>
void separateOddsAndEvens(
    const int arr[],
    size_t size,
    int** odds,
    size_t* numOdds,
...
// Copy the odds and evens to the new arrays.
	...
	for (size_t i = 0; i < size; ++i) {
		...
		if (arr[i] % 2 == 1) { (*odds)[oddsPos++] = arr[i]; } else {
...
// Use the arrays...
	std::cout << numOdds   << " " <<  oddNums[numOdds - 1] << std::endl;
	std::cout << numEvens << " " << evenNums[numEvens - 1] << std::endl;
